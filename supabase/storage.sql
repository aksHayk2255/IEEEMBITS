...
-- Replace only this project's policy names. Existing bucket files are preserved.
drop policy if exists "public read managed images" on storage.objects;
drop policy if exists "admins upload managed images" on storage.objects;
...
create policy "public read managed images" on storage.objects for select
  using (bucket_id in ('team','events','projects','achievements','gallery'));
create policy "admins upload managed images" on storage.objects for insert to authenticated
  with check (public.is_admin() and bucket_id in ('team','events','projects','achievements','gallery'));
create policy "admins update managed images" on storage.objects for update to authenticated
  using (public.is_admin() and bucket_id in ('team','events','projects','achievements','gallery'))
  with check (public.is_admin() and bucket_id in ('team','events','projects','achievements','gallery'));
