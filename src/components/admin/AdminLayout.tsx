...
const links = [
  ['/', 'Overview'], ['/admin/events', 'Events'], ['/admin/projects', 'Projects'],
...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-bg text-ink lg:flex">
    <aside className="border-b border-line bg-panel lg:min-h-screen lg:w-64 lg:border-r lg:border-b-0">
      ...
      <nav className="overflow-x-auto px-3 pb-3 lg:px-4 lg:pb-6"><ul className="flex gap-1 lg:block">
        {links.map(([to, label]) => <li key={to}><NavLink end={to === '/'} to={to} className={({ isActive }) => `flex items-center gap-3 whitespace-nowrap rounded-sm px-3 py-2.5 text-sm ${isActive ? 'bg-accent text-bg' : 'text-muted hover:bg-surface hover:text-ink'}`}>
          {to === '/' ? <LayoutDashboard size={16} /> : to === '/admin/settings' ? <Settings size={16} /> : <Table2 size={16} />}{label}
        </NavLink></li>)}
      </ul></nav>
    </aside>
